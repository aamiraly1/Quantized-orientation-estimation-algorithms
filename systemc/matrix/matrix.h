// Matrix class from https://github.com/hitc8494/Matrix
// Template part added
#ifndef __MATRIX_H__
#define __MATRIX_H__
...
class Matrix {
 public:
  ...
  Matrix multiplyRow( const T d, const uint row );
  Matrix& ADDToRow( const Matrix& rhs, const uint row );
  Matrix& SWITCHRows( const uint row1, const uint row2 );
  Matrix& MULTIPLYRow( const T d, const uint row );
  ...
  Matrix operator * ( const Matrix& rhs );//
  Matrix mul( const T& in );        // Scalar multiplication
  Matrix operator * ( const T& in );//
  Matrix& MUL( const Matrix& rhs );         // Immediate multiplication
  Matrix& operator *= ( const Matrix& rhs );//
  Matrix& MUL( const T& in );         // Immediate scalar multiplication
  Matrix& operator *= ( const T& in );//
  Matrix div( const Matrix& rhs );        // Division
  Matrix operator / ( const Matrix& rhs );//
  Matrix div( const T& in );        // Scalar division
  Matrix operator / ( const T& in );//
  Matrix& DIV( const Matrix& rhs );         // Immediate division
  Matrix& operator /= ( const Matrix& rhs );//
  Matrix& DIV( const T& in );         // Immediate scalar division
  Matrix& operator /= ( const T& in );//
 ...
 private:
  ...
  T _det( Matrix in ); //Auxiliary recursive function to calculate determinant.
